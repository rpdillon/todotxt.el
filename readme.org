...
* Limitations
  ...
  - There's no indication that the list is filtered.  If you're unsure
    ...
    all the items.
* Known Bugs
  - There's a pretty serious bug with the hiding code that is causing
    line-specific functions to sometimes grab the wrong text when
    commands like =e= are issued, resulting in blank text.
